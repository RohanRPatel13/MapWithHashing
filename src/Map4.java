...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        // Initialize each entry of the hashTable.
        for (int i = 0; i < hashTableSize; i++) {
            this.hashTable.setEntry(i, new Map2<K, V>());
        }
        this.size = 0; // set size to its initial value.
    }
    ...
    public Map4(int hashTableSize) {
        ...
        assert hashTableSize > 0 : "violation: at least one bucket.";
    ...
    public final void add(K key, V value) {
        ...
        this.hashTable.entry(index).add(key, value); // add to the bucket.
    ...
    public final Pair<K, V> removeAny() {
        ...
        // Implemented 05/06.
        int i = 0;
        while (this.hashTable.entry(i).size() == 0) {
            i++;
        }
        this.size--;
        return this.hashTable.entry(i).removeAny();
    }
    ...
    public final boolean hasKey(K key) {
        ...
        // Implemented 04/06.
        int index = mod(key.hashCode(), this.hashTable.length());
        return this.hashTable.entry(index).hasKey(key);
    }
